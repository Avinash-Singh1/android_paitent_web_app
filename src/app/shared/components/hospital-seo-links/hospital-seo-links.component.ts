...
@Component({
  standalone: false,
  selector: "nectar-hospital-seo-links",
  templateUrl: "./hospital-seo-links.component.html",
  styleUrls: ["./hospital-seo-links.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class HospitalSeoLinksComponent implements OnInit, OnChanges, OnDestroy {
  @Input() city = "";
  @Input() locality = "";

  topLocalities: string[] = [];
  topCities: string[] = [];
  topSpecializations: { slug: string; hospitalType: string }[] = [];
  ...
  private loadLinks(): void {
    this.loaded = true;
    this.http
      .post<any>(API_ENDPOINTS.patient.hospitalSeoLinks, { city: this.city })
      ...
      .subscribe({
        ...
        error: () => {
          // Silently fail — SEO links are non-critical
        },
      });
  }
  ...
  getLocalityLink(loc: string): any[] {
    return ["/", this.formatSlug(this.city), "hospitals", this.formatSlug(loc)];
  }

  getCityHospitalsLink(cityName: string): any[] {
    return ["/", this.formatSlug(cityName), "hospitals"];
  }

  getSpecLink(slug: string): any[] {
    return ["/", this.formatSlug(this.city), "hospitals", slug];
  }
  ...
  getCityClinicsLink(): any[] {
    return ["/", this.formatSlug(this.city), "clinics"];
  }
