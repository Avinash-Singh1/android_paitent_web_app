...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  private updateUsesCache(): void {
    this._usesList = this.getUsesList();
    this.usesOverflowCount = this._usesList.length > 3 ? this._usesList.length - 3 : 0;
    this.visibleUses = this.showAllUses ? this._usesList : this._usesList.slice(0, 3);
  }

  getVisibleUses(): string[] {
    return this.visibleUses;
  }

  getUsesOverflowCount(): number {
    return this.usesOverflowCount;
  }

  toggleUses(): void {
    this.showAllUses = !this.showAllUses;
    this.updateUsesCache();
  }

  toggleWishlist(): void {
    this.isWishlisted = !this.isWishlisted;
    this.medicineStateService.toggleWishlist(this.productDetails);
  }
    ...
    this.medicineApiService.getMedicineBySlugOrId(id.toString())
      ...
      .subscribe({
        next: (medicine) => {
          ...
          // Reset tab and check wishlist state
          this.activeTab = 'description';
          this.isWishlisted = this.medicineStateService.isInWishlist(medicine._id);
